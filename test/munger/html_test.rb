...
describe Remunger::Render::Html do
  ...
  it "must accept a Remunger::Report object" do
    Remunger::Render::Html.new(@report.process).must_be :valid?
  end

  it "must render a basic html table" do
    @render = Remunger::Render::Html.new(@report.process)
    count = @report.rows
  ...
  it "must accept a custom table class" do
    rep = Remunger::Render::Html.new(@report.process, :classes => {:table => 'helloClass'})
    html = rep.render
